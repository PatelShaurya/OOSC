"""
CLI tool for testing Stage 4A/4B semantic retrieval with optional metadata filtering.
"""
import argparse
import json
import sys
...
def format_cli_output(response):
    ...
    for idx, item in enumerate(response.results, 1):
        ...
        print(f"Type: {item.document_type} | Authority: {item.issuing_authority}")
...
def main():
    parser = argparse.ArgumentParser(description="Stage 4B Semantic Retrieval CLI with Metadata Filtering.")
    parser.add_argument("--query", type=str, required=True, help="User natural-language query")
    parser.add_argument("--top-k", type=int, default=5, help="Number of top relevant chunks to retrieve")
